...
!function() {
    ...
    $.extend({_jsonp: {scripts: {},counter: 1,charset: "GBK",head: document.getElementsByTagName("head")[0],name: function(callback) {
...
var category = {OBJ: $("#_JD_ALLSORT"),
		//URL_Serv: "http://manage.dada.com/web/itemcat/all?callback=category.getDataService",
        ...
        return "" == c ? d : (/^http[s]?:\/\/([\w-]+\.)+[\w-]+([\w-.\/?%&=]*)?$/.test(c) || (c = c.replace(/-000$/, ""), c = c.match(/^\d*-\d*$/) ? "http://channel.jd.com/" + c + ".html" : "https://www.jd.com/" + c + ".html"), '<a href="' + c + '">' + d + "</a>")
